...
// event
void chen::tcp::server::onEventRead(std::vector<std::uint8_t> data, std::error_code error)
{
    if (error)
        return this->stop();
...
void chen::tcp::server::onEventWrite(std::size_t size, std::error_code error)
{
    // unused
}

void chen::tcp::server::onEventEnd()
{
    // stop if error occur
    this->stop();
}
