...
std::uint16_t chen::net::resolver::service(const std::string &name)
{
    auto ent = ::getservbyname(name.c_str(), nullptr);
    return ent ? num::swap(static_cast<std::uint16_t>(ent->s_port)) : static_cast<std::uint16_t>(std::atoi(name.c_str()));
}
